...
class _CQCLocationValue:
    ...
    saf_assessment_json: str  


# ==========================================
# 3. CUSTOM ASYNC SOURCE CONNECTOR
# ==========================================
...
class CQCConnector:
    ...
    async def list(self) -> AsyncIterator[PartialSourceRow[_CQCLocationKey, _CQCLocationValue]]:
        # Using the "Sniper" ID to guarantee a hit during testing
        known_saf_ids = ["1-6520249041"] 
        ...
        async with cqc_rate_limiter:
            ...
            async with self._session.get(url, headers=self._headers) as response:
                ...
                val = _CQCLocationValue(
                    ...
                    saf_assessment_json=json.dumps(data.get("assessment", []))
                )
...
@cocoindex.op.function()
def generate_uuid(trigger: Any) -> str:
    return str(uuid.uuid4())

@cocoindex.op.function()
def get_reg_for_qs_id(qs_id: str) -> int:
    # Use the dictionary to map perfectly to init_db.py static integer IDs
    return QS_ID_TO_REGULATION.get(str(qs_id), 0)    


# ==========================================
# 5. THE DECLARATIVE GRAPH FLOW
# ==========================================
...
def docs_to_kg_flow(flow_builder: cocoindex.FlowBuilder, data_scope: cocoindex.DataScope) -> None:
    ...
    with data_scope["cqc_locations"].row() as doc:
        ...
        doc["kq_assessments"] = doc["saf_assessment_json"].transform(unwrap_saf_json)
        ...
        with doc["kq_assessments"].row() as kq_asmt:
            ...
            with kq_asmt["qs_assessments"].row() as qs_asmt:
                ...
                # 1. Transform the ID to find the Regulation
                qs_asmt["mapped_reg_id"] = qs_asmt["qs_id"].transform(get_reg_for_qs_id)
                
                # 2. Collect the edge using flat kwargs
                edge_qs_reg.collect(
                    ...
                    item["extracted_data"] = item["html_commentary"].transform(
                        cocoindex.functions.ExtractByLlm(
                            ...
                            instruction=(
                                ...
                                "- ALLOW EMPTY: If the text contains no definitive risks or positive practices, return empty lists. Do not force extractions from generic filler text."
                            )
                        )
                    )
                    
                    with item["extracted_data"]["identified_risks"].row() as risk:
                        risk["risk_uuid"] = doc["location_id"].transform(generate_uuid)
                        ...
                        risk_node.collect(risk_id=risk["risk_uuid"], description=risk["description"], embedding=risk["embedding"])
                        
                        # Link 1: The Audit Trail
                        edge_evidence_risk.collect(edge_id=cocoindex.GeneratedField.UUID, ev_id=item["evidence_id"], rsk_id=risk["risk_uuid"])
                        # Link 2: The Dynamic Care Home Assessment
                        edge_qs_risk.collect(edge_id=cocoindex.GeneratedField.UUID, qs_asmt_id=qs_asmt["qs_asmt_id"], rsk_id=risk["risk_uuid"])

                    with item["extracted_data"]["positive_practices"].row() as practice:
                        practice["prac_uuid"] = doc["location_id"].transform(generate_uuid)
                        practice["embedding"] = practice["description"].transform(
                            cocoindex.functions.SentenceTransformerEmbed(model="google/embeddinggemma-300m", args={"token": HF_TOKEN})
                        )
                        
                        practice_node.collect(practice_id=practice["prac_uuid"], description=practice["description"], embedding=practice["embedding"])
                        
                        # Link 1: The Audit Trail
                        edge_evidence_practice.collect(edge_id=cocoindex.GeneratedField.UUID, ev_id=item["evidence_id"], prac_id=practice["prac_uuid"])
                        # Link 2: The Dynamic Care Home Assessment
                        edge_qs_practice.collect(edge_id=cocoindex.GeneratedField.UUID, qs_asmt_id=qs_asmt["qs_asmt_id"], prac_id=practice["prac_uuid"])
    ...
    location_node.export("exp_loc", GraphDbSpec(connection=conn_spec, mapping=cocoindex.targets.Nodes(label="Location")),primary_key_fields=["location_id"])
